...
@section('title', 'Master Jenis Tes')
@section('content_header')
    <div class="row">
        <div class="mb-3 col-12">
            <h1 class="m-0 font-bold text-dark"><i class="fa fa-book" aria-hidden="true"></i> Master Jenis Tes</h1>
        </div>
    </div>
@endsection
@section('content')
                                        ...
                                        <div class="dropdown open">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="triggerId">
                                                <a class="dropdown-item ubah-data" href="#modalUbah" data-toggle="modal"
                                                    data-update="{{ route('jenis-tes.update', encrypt($data->id_ref_jenis_tes)) }}"
                                                    data-edit="{{ route('jenis-tes.edit', encrypt($data->id_ref_jenis_tes)) }}"><i
                                                        class="fa fa-edit" aria-hidden="true"></i> Ubah</a>

                                                <a class="dropdown-item"
                                                    onclick="confirmation('jenis_{{ $data->id_ref_jenis_tes }}')"><i
                                                        class="fa fa-trash" aria-hidden="true"></i> Hapus
                                                    <form
                                                        action="{{ route('jenis-tes.destroy', encrypt($data->id_ref_jenis_tes)) }}"
                                                        method="post" id="jenis_{{ $data->id_ref_jenis_tes }}">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('jenis-tes.store') }}" method="post" id="form-kategori">
                        @csrf
