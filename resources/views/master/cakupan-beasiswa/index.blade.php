...
@section('title', 'Master Cakupan Beasiswa')
@section('content_header')
    <div class="row">
        <div class="mb-3 col-12">
            <h1 class="m-0 font-bold text-dark"><i class="fa fa-book" aria-hidden="true"></i> Master Cakupan Beasiswa</h1>
        </div>
    </div>
@endsection
@section('content')
                ...
                <div class="card-body">
                    ...
                    <table class="table" id="table-bobot">
                        <thead class="thead-dark">
                            <tr>
                                <th>#</th>
                                <th>Nama Cakupan Beasiswa</th>
                                <th><i class="fa fa-cogs" aria-hidden="true"></i></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($data['pokok'] as $dataCakupanBeasiswa)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $dataCakupanBeasiswa->nama }}</td>
                                    <td>
                                        <div class="dropdown open">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="triggerId">
                                                <a class="dropdown-item ubah-data" href="#modalUbah" data-toggle="modal"
                                                    data-update="{{ route('cakupan-beasiswa.update', encrypt($dataCakupanBeasiswa->id_ref_cakupan_beasiswa)) }}"
                                                    data-edit="{{ route('cakupan-beasiswa.edit', encrypt($dataCakupanBeasiswa->id_ref_cakupan_beasiswa)) }}"><i
                                                        class="fa fa-edit" aria-hidden="true"></i> Ubah</a>

                                                <a class="dropdown-item"
                                                    onclick="confirmation('cakupan_beasiswa_{{ $dataCakupanBeasiswa->id_ref_cakupan_beasiswa }}')"><i
                                                        class="fa fa-trash" aria-hidden="true"></i> Hapus
                                                    <form
                                                        action="{{ route('cakupan-beasiswa.destroy', encrypt($dataCakupanBeasiswa->id_ref_cakupan_beasiswa)) }}"
                                                        method="post" id="cakupan_beasiswa_{{ $dataCakupanBeasiswa->id_ref_cakupan_beasiswa }}">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('cakupan-beasiswa.store') }}" method="post" id="form-kategori">
                        @csrf
                        <div class="form-group">
                            <label for="">Nama Cakupan Beasiswa</label>
                            <input type="text" class="form-control" name="nama" id="" aria-describedby="helpId"
                                placeholder="Nama Cakupan Beasiswa">
                        </div>
