...
@section('title', 'Master Level Bahasa')
@section('content_header')
    <div class="row">
        <div class="mb-3 col-12">
            <h1 class="m-0 font-bold text-dark"><i class="fa fa-book" aria-hidden="true"></i> Master Level Bahasa</h1>
        </div>
    </div>
@endsection
@section('content')
                ...
                <div class="card-body">
                    ...
                    <table class="table" id="table-bobot">
                        <thead class="thead-dark">
                            <tr>
                                <th>#</th>
                                <th>Nama Level Bahasa</th>
                                <th><i class="fa fa-cogs" aria-hidden="true"></i></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($data['pokok'] as $data)
                                <tr>
                                        ...
                                        <div class="dropdown open">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="triggerId">
                                                <a class="dropdown-item ubah-data" href="#modalUbah" data-toggle="modal"
                                                    data-update="{{ route('level-bahasa.update', encrypt($data->id_ref_level_bahasa)) }}"
                                                    data-edit="{{ route('level-bahasa.edit', encrypt($data->id_ref_level_bahasa)) }}"><i
                                                        class="fa fa-edit" aria-hidden="true"></i> Ubah</a>

                                                <a class="dropdown-item"
                                                    onclick="confirmation('level_bahasa_{{ $data->id_ref_level_bahasa }}')"><i
                                                        class="fa fa-trash" aria-hidden="true"></i> Hapus
                                                    <form
                                                        action="{{ route('level-bahasa.destroy', encrypt($data->id_ref_level_bahasa)) }}"
                                                        method="post" id="level_bahasa_{{ $data->id_ref_level_bahasa }}">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('level-bahasa.store') }}" method="post" id="form-kategori">
                        @csrf
                        <div class="form-group">
                            <label for="">Nama Level Bahasa</label>
                            <input type="text" class="form-control" name="nama" id="" aria-describedby="helpId"
                                placeholder="Nama Level Bahasa">
                        </div>
